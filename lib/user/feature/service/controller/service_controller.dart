...
import 'package:mstoo/user/feature/service/model/recommendation_search_model.dart';
import 'package:get/get.dart';
import 'package:mstoo/user/core/core_export.dart';
import 'package:mstoo/user/feature/campaign/model/service_types.dart';
...
class ServiceController extends GetxController implements GetxService {
  ...
  Future<void> getSubCategoryBasedServiceList(String subCategoryID, bool isWithPagination, {bool isShouldUpdate = true, bool showShimmerAlways = false}) async {
    ...
    Future<void> sortByPriceAsec(String subCategoryID, bool isWithPagination, {bool isShouldUpdate = true, bool showShimmerAlways = false}) async {
    if(showShimmerAlways){
      _subCategoryBasedServiceList = null;
      update();
    }
    Response response = await serviceRepo.sortByPriceAsec(subCategoryID: subCategoryID,offset: 1);
    if (response.statusCode == 200) {
      if(!isWithPagination){
        _subCategoryBasedServiceList = [];
      }
  ...
  Future<void> sortByPriceDesc(String subCategoryID, bool isWithPagination, {bool isShouldUpdate = true, bool showShimmerAlways = false}) async {
    if(showShimmerAlways){
      _subCategoryBasedServiceList = null;
      update();
    }
    Response response = await serviceRepo.sortByPriceDesc(subCategoryID: subCategoryID,offset: 1);
    if (response.statusCode == 200) {
      if(!isWithPagination){
        _subCategoryBasedServiceList = [];
      }
  ...
  Future<void> sortByOldest(String subCategoryID, bool isWithPagination, {bool isShouldUpdate = true, bool showShimmerAlways = false}) async {
    if(showShimmerAlways){
      _subCategoryBasedServiceList = null;
      update();
    }
    Response response = await serviceRepo.sortByOldest(subCategoryID: subCategoryID,offset: 1);
    if (response.statusCode == 200) {
      if(!isWithPagination){
        _subCategoryBasedServiceList = [];
      }
  ...
  // sort all services by price low to high
  Future<void> sortAllServiceByPriceAsec(int offset, bool reload) async {
    if(offset != 1 || _allService == null || reload){
      if(reload){
        _allService = null;
      }
      Response response = await serviceRepo.sortAllServiceByPriceAsec(offset);
      if (response.statusCode == 200) {
        if(reload){
          _allService = [];
        }
  ...
  // sort all services by price high to low
  Future<void> sortAllServiceByPriceDesc(int offset, bool reload) async {
    if(offset != 1 || _allService == null || reload){
      if(reload){
        _allService = null;
      }
      Response response = await serviceRepo.sortAllServiceByPriceDesc(offset);
      if (response.statusCode == 200) {
        if(reload){
          _allService = [];
        }
  ...
  // sort all services by oldest
  Future<void> sortAllServiceByOldest(int offset, bool reload) async {
    if(offset != 1 || _allService == null || reload){
      if(reload){
        _allService = null;
      }
      Response response = await serviceRepo.sortAllServiceByOldest(offset);
      if (response.statusCode == 200) {
        if(reload){
          _allService = [];
        }
