...
class BookingDetailsTabsController extends GetxController
  ...
  bool get isRejectButtonLoading => _isRejectButtonLoading;
...
// custom function by naresh for change schedule in user
  ...
  Future<void> markCompleted() async {
    ...
    Future<void> markCompletedByRenter() async {
    Response response = await bookingDetailsRepo.markCompletedByRenter(bookingID : bookingDetailsContent!.id!);
    if (response.statusCode == 200) {
      // getBookingDetailsData(_bookingDetailsContent!.id!,reload: false);
      customSnackBar("Booking marked as completed successfully",isError: false);
    } else {
  ...
  Future<void> acceptBookingRequest(String bookingId) async {
    ...
    if(response.statusCode==200){
      await getBookingDetails(bookingId: bookingId);
      customSnackBar("Booking accepted successfully", isError: false);
    }
    else{
  ...
  Future<void> rejectBookingRequest(String bookingId) async {
    _isRejectButtonLoading = true;
    update();
    Response response = await bookingDetailsRepo!.rejectBookingRequest(bookingId);
    if(response.statusCode==200){
       await getBookingDetails(bookingId: bookingId);
         customSnackBar("Booking rejected successfully", isError: true);
    }
    else{
      customSnackBar(response.statusText);
    }
    _isRejectButtonLoading = false;
    update();
  }
