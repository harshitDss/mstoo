...
class BookingDetailsRepo {
  ...
  Future<Response> markCompletedByRenter({required String bookingID}) async {
    return await apiClient.postData('${AppConstants.markCompletedByRenter}/$bookingID',
        {"booking_status": "completed", "_method": "put"});
  }

    Future<Response> acceptBookingRequest(String bookingID) async {
    return await apiClient.putData("${AppConstants.bookingAcceptStatusUrl}/$bookingID",{'method':'put'});
  }

  Future<Response> rejectBookingRequest(String bookingID) async {
    return await apiClient.putData("${AppConstants.bookingAcceptStatusUrl}/$bookingID",{'method':'put'});
  }
  //closed
}
